...
fn run(s: &Seq) {
    ...
    let v = if body.validate().is_ok() && err < 2e-2 {
        "PASS-CORRECT"
    } else {
        "** CHECK **"
    };
    println!(
        "{:<26} {v}  mass={m:.3} mesh={mesh:.3} MC={truth:.3} m-MC={err:.4}",
        s.label
    );
}
