...
fn make(rng: &mut Rng) -> (&'static str, Body, Body, BoolOp) {
    ...
    match t {
        0 => {
            ...
            ("through-hole", blk(Vec3::ZERO, w, d, h), cyl(Vec3::new(x, y, -0.5), z, r, h + 1.0), BoolOp::Difference)
        }
        1 => {
            ...
            ("blind-hole", blk(Vec3::ZERO, w, d, h), cyl(Vec3::new(x, y, h - depth), z, r, depth + 0.6), BoolOp::Difference)
        }
        2 => {
            ...
            ("pocket", blk(Vec3::ZERO, w, d, h), blk(Vec3::new(px, py, h - depth), pw, pd, depth + 0.6), BoolOp::Difference)
        }
        3 => {
            ...
            ("slot", blk(Vec3::ZERO, w, d, h), blk(Vec3::new(px, -0.5, -0.5), sw, d + 1.0, h + 1.0), BoolOp::Difference)
        }
        4 => {
            let (w, d, h) = (rng.r(4., 9.), rng.r(4., 9.), rng.r(2., 5.));
            let (cw, cd, ch) = (rng.r(1., w * 0.5), rng.r(1., d * 0.5), rng.r(1., h * 0.9));
            ("corner-notch", blk(Vec3::ZERO, w, d, h), blk(Vec3::new(w - cw, d - cd, h - ch), cw + 0.5, cd + 0.5, ch + 0.5), BoolOp::Difference)
        }
        5 => {
            let (w, d, h) = (rng.r(4., 9.), rng.r(4., 9.), rng.r(1., 3.));
            let r = rng.r(0.5, w.min(d) / 3.0);
            let (x, y) = (rng.r(r + 0.3, w - r - 0.3), rng.r(r + 0.3, d - r - 0.3));
            ("boss-cyl", blk(Vec3::ZERO, w, d, h), cyl(Vec3::new(x, y, h - 0.02), z, r, rng.r(0.8, 3.0)), BoolOp::Union)
        }
        6 => {
            let (w, d, h) = (rng.r(5., 10.), rng.r(5., 10.), rng.r(1., 3.));
            let (bw, bd) = (rng.r(1., w * 0.5), rng.r(1., d * 0.5));
            let (x, y) = (rng.r(0.3, w - bw - 0.3), rng.r(0.3, d - bd - 0.3));
            ("boss-block", blk(Vec3::ZERO, w, d, h), blk(Vec3::new(x, y, h - 0.02), bw, bd, rng.r(0.8, 3.0)), BoolOp::Union)
        }
        7 => {
            let (rr, h) = (rng.r(1.5, 4.), rng.r(2., 8.));
            let r = rng.r(0.4, rr * 0.8);
            ("tube-bore", cyl(Vec3::ZERO, z, rr, h), cyl(Vec3::new(0., 0., -0.5), z, r, h + 1.0), BoolOp::Difference)
        }
        8 => {
            let (rr, h) = (rng.r(0.6, 2.5), rng.r(3., 8.));
            let cr = rng.r(rr + 0.4, rr + 2.5);
            let cz = rng.r(0.5, h - 1.5);
            ("collar", cyl(Vec3::ZERO, z, rr, h), cyl(Vec3::new(0., 0., cz), z, cr, rng.r(0.5, 1.5)), BoolOp::Union)
        }
        9 => {
            ...
            ("countersink", blk(Vec3::ZERO, w, d, h), cone(Vec3::new(x, y, h + 0.05), Vec3::new(0., 0., -1.), r, depth + 0.05), BoolOp::Difference)
        }
        10 => {
            let (rr, h) = (rng.r(1., 3.), rng.r(2., 6.));
            ("rounded-end", cyl(Vec3::ZERO, z, rr, h), sph(Vec3::new(0., 0., h), rr), BoolOp::Union)
        }
        11 => {
            let (w, d, h) = (rng.r(5., 10.), rng.r(5., 10.), rng.r(1., 3.));
            let r = rng.r(1., w.min(d) / 3.0);
            ("dome", blk(Vec3::ZERO, w, d, h), sph(Vec3::new(w / 2.0, d / 2.0, h), r), BoolOp::Union)
        }
        12 => {
            let (w, d, h) = (rng.r(5., 10.), rng.r(5., 10.), rng.r(2., 4.));
            let r = rng.r(1., (w.min(d) / 3.0).min(h));
            ("ball-cavity", blk(Vec3::ZERO, w, d, h), sph(Vec3::new(w / 2.0, d / 2.0, h - r * 0.4), r), BoolOp::Difference)
        }
        _ => {
            ...
            ("cross-hole", cyl(Vec3::ZERO, z, rr, h), cyl(Vec3::new(-rr - 0.5, 0., cz), Vec3::new(1., 0., 0.), r, 2.0 * rr + 1.0), BoolOp::Difference)
        }
    }
}

fn main() {
    let n: usize = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(30000);
    ...
    println!("{:<16} {:>6} {:>7} {:>8} {:>6}", "template", "ops", "PASS%", "DECLINE%", "FAIL");
    for l in &labels {
        let (t, p, d, f) = (tot[l], *pass.get(l).unwrap_or(&0), *decl.get(l).unwrap_or(&0), *fail.get(l).unwrap_or(&0));
        tp += p;
        td += d;
        tf += f;
        println!("{:<16} {:>6} {:>6.1} {:>7.1} {:>6}", l, t, 100.0 * p as f64 / t as f64, 100.0 * d as f64 / t as f64, f);
    }
    let nt = n as f64;
    println!("---");
    println!("OVERALL: PASS {tp} ({:.1}%)  DECLINE {td} ({:.1}%)  FAIL {tf} ({:.2}%)", 100.0 * tp as f64 / nt, 100.0 * td as f64 / nt, 100.0 * tf as f64 / nt);
}
