...
#[derive(Clone)]
enum Tool {
    Blk { o: Vec3, dx: f64, dy: f64, dz: f64 },
    Cyl { pos: Vec3, axis: Vec3, r: f64, h: f64 },
    Cone { pos: Vec3, axis: Vec3, r: f64, h: f64 },
    Sph { c: Vec3, r: f64 },
}
fn v(p: Vec3) -> String {
...
impl Tool {
    fn build(&self) -> Body {
        ...
        match *self {
            ...
            Tool::Cyl { pos, axis, r, h } => b.cylinder(Frame3::from_z(pos, axis).unwrap(), r, h).unwrap(),
            Tool::Cone { pos, axis, r, h } => b.cone(Frame3::from_z(pos, axis).unwrap(), r, h).unwrap(),
            Tool::Sph { c, r } => b.sphere(Frame3::from_z(c, Vec3::new(0., 0., 1.)).unwrap(), r).unwrap(),
        };
        b
    }
    fn code(&self) -> String {
        match *self {
            Tool::Blk { o, dx, dy, dz } => format!("blk({}, {:.3}, {:.3}, {:.3})", v(o), dx, dy, dz),
            Tool::Cyl { pos, axis, r, h } => format!("cyl({}, {}, {:.3}, {:.3})", v(pos), v(axis), r, h),
            Tool::Cone { pos, axis, r, h } => format!("cone({}, {}, {:.3}, {:.3})", v(pos), v(axis), r, h),
...
fn feature(rng: &mut Rng, cx: f64, cy: f64, s: f64, h: f64) -> Feat {
    ...
    let table: &[(u32, u8)] = &[(22, 0), (12, 1), (14, 2), (14, 3), (8, 4), (8, 5), (8, 6), (7, 7), (7, 8)];
    ...
    match t {
        0 => Feat { label: "through-hole", tool: Tool::Cyl { pos: Vec3::new(x, y, -0.5), axis: z, r: rng.r(0.3, rmax), h: h + 1.0 }, op: BoolOp::Difference },
        1 => {
            let dep = rng.r(0.3, h * 0.7);
            Feat { label: "blind-hole", tool: Tool::Cyl { pos: Vec3::new(x, y, h - dep), axis: z, r: rng.r(0.3, rmax), h: dep + 0.6 }, op: BoolOp::Difference }
        }
        2 => {
            let (pw, pd) = (rng.r(0.6, s), rng.r(0.6, s));
            let dep = rng.r(0.3, h * 0.7);
            Feat { label: "pocket", tool: Tool::Blk { o: Vec3::new(x - pw / 2.0, y - pd / 2.0, h - dep), dx: pw, dy: pd, dz: dep + 0.6 }, op: BoolOp::Difference }
        }
        3 => Feat { label: "boss-cyl", tool: Tool::Cyl { pos: Vec3::new(x, y, h - 0.02), axis: z, r: rng.r(0.3, rmax), h: rng.r(0.6, 2.5) }, op: BoolOp::Union },
        4 => {
            let (bw, bd) = (rng.r(0.6, s), rng.r(0.6, s));
            Feat { label: "boss-block", tool: Tool::Blk { o: Vec3::new(x - bw / 2.0, y - bd / 2.0, h - 0.02), dx: bw, dy: bd, dz: rng.r(0.6, 2.5) }, op: BoolOp::Union }
        }
        5 => {
            let dep = rng.r(0.4, h * 0.6);
            Feat { label: "countersink", tool: Tool::Cone { pos: Vec3::new(x, y, h + 0.05), axis: Vec3::new(0., 0., -1.), r: rng.r(0.5, rmax), h: dep + 0.05 }, op: BoolOp::Difference }
        }
        6 => Feat { label: "dome", tool: Tool::Sph { c: Vec3::new(x, y, h), r: rng.r(0.5, rmax) }, op: BoolOp::Union },
        7 => {
            let r = rng.r(0.5, rmax.min(h));
            Feat { label: "ball-cavity", tool: Tool::Sph { c: Vec3::new(x, y, h - r * 0.4), r }, op: BoolOp::Difference }
        }
        _ => {
            let r = rng.r(0.25, rmax.min(h / 2.5));
            let zc = rng.r(r + 0.2, h - r - 0.2).max(r + 0.2);
            Feat { label: "cross-hole", tool: Tool::Cyl { pos: Vec3::new(x - s - 0.5, y, zc), axis: Vec3::new(1., 0., 0.), r, h: 2.0 * s + 1.0 }, op: BoolOp::Difference }
        }
    }
}
...
fn main() {
    let parts: usize = std::env::args().nth(1).and_then(|s| s.parse().ok()).unwrap_or(300);
    ...
    for _ in 0..parts {
        ...
        let stock = Tool::Blk { o: Vec3::ZERO, dx: w, dy: d, dz: h };
        let mut body = stock.build();
        let mut landed: Vec<Feat> = Vec::new();
        for ix in 0..nx {
            for iy in 0..ny {
                let f = feature(&mut rng, (ix as f64 + 0.5) * cell, (iy as f64 + 0.5) * cell, s, h);
    ...
    println!("=== BUG EXTRACT ({parts} parts, seed {seed}): {total_fail} failing ops, {} distinct gaps ===\n", v.len());
    for (i, (key, hit)) in v.iter().enumerate().take(14) {
        ...
        for f in &minp {
            println!("     b = boolean(&b, &{}, BoolOp::{}, 1e-7).unwrap().body; // {}", f.tool.code(), opname(f.op), f.label);
        }
        println!("     // FAILS: boolean(&b, &{}, BoolOp::{}, 1e-7)  [{}]", hit.fail.tool.code(), opname(hit.fail.op), hit.fail.label);
        println!();
    }
}
