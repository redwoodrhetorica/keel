//! Sharded WRONG-scanner for the three-bucket oracle's strict lane:
//! replays the oracle's exact LCG trials over [KEEL_SCAN_START,
//! KEEL_SCAN_END) and prints every strict-lane WRONG with full
//! context. Run shards in parallel processes to localize a WRONG
//! found by a long single-threaded oracle run without repaying its
//! wall-clock. Judge logic mirrors three_bucket.rs exactly.
...
#[test]
#[ignore = "diagnosis shard; run with KEEL_SCAN_START/KEEL_SCAN_END"]
fn scan_strict_wrong() {
    ...
    for trial in start..end {
        ...
        let Ok(res) = boolean(&a, &b, op, 1e-7) else {
            continue;
        };
        ...
        let ok = match mass {
            Ok(m) => (m - reference).abs() <= tol && (mesh - reference).abs() <= mesh_tol,
            Err(_) => reference <= 1e-9 + contact_slack && mesh.abs() <= 1e-9 + contact_slack,
        };
        if !ok {
            wrongs += 1;
            eprintln!(
                "WRONG trial {trial} {op:?} contact {contact} d{delta:+.0e} ax{axis}: ref {reference}, mass {mass:?}, mesh {mesh}, a {a0:?}+{ad:?}, b {b0:?}+{bd:?}"
            );
        }
    }
    eprintln!("scan [{start},{end}): {wrongs} wrong");
}
