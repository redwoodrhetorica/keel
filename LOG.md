...
## Addendum 295: the non-coaxial cyl/sphere WRAP passes all 3 ops -- the sphere fragments were never SPLIT (a relocation gap), not a mass mis-route (dossiers 68/69) (2026-06-18)

The dominant decline class's hardest sub-case (non-coaxial sphere-vs-{cyl,cone} WRAP) now PASSES all 3 ops. Implemented per dossiers 68/69 by a focused fresh-context worktree agent, then independently verified and integrated (commit c321f9a, branch decline-frontier).

ROOT (the diagnosis CORRECTED the dossier premise). The dossiers assumed the sphere band/cap faces EXISTED and the green-slab mis-routed them (the iso closed_cover path). The real root was UPSTREAM: the sphere was never split into fragments. probe_wrap's off-axis equatorial rod cuts the sphere in two CONTRACTIBLE NURBS rings (each encircles neither the rod axis nor a pole), so the decomposition is "rest-with-2-holes + 2 caps", NOT the pole-encircling "band + 2 caps" the general dossier describes. The 2nd ring was DROPPED in the multi-component imprint relocation: curved_face_containing could not locate a fragment of the pristine/holed sphere (sphere_face_cut_axis returned the meridian-seam axis; the latitude-band test failed). With one ring imprinted, classify/select/stitch dropped or whole-kept the sphere.

FIX (boolean.rs +162, tessellate.rs +162; NO mass-integrator change). (1) RELOCATION: curved_face_containing's sphere arm now accepts a full sphere via face_covers_closed_surface + point_in_face_uv, with a frame-free sphere_face_containing_3d (solid-angle winding, Van Oosterom-Strackee, loop_solid_angle_about) as the final fallback. Once BOTH rings imprint, the EXISTING green-slab integrates the NURBS-rim faces correctly -- no mass change (my own prior sphere band-flip was the WRONG layer: it regressed the soak and gave wrong mass, reverted). (2) TESSELLATION (dossier 69): the multi-hole sphere rest face over-read and was non-watertight; each NURBS hole now builds a cone of facet planes from the sphere centre through its rim chords (shared fin_curve_samples) and grid triangles are differenced (subtract_cone), so the mesh terminates on the rim chords -- watertight with the rod-stub wall by construction (gated >=2 holes, single-window baseline byte-unchanged).

CERTIFIED, not merely self-consistent: the independent tight oracle quadric_sphere_op_volume gates each op and matches the mass (dossier 68 sec 5.3). probe_wrap (KEEL_WRAP_FLOW=1): I 17.164/17.134/MC17.174, D 7.969/7.948/7.951, U 121.066/120.651/121.000 -- all within 0.3%, open_ratio ~0.

GATE: curved_volume_robustness 11/11; keel-topo lib+tests 314 passed/0 failed; soak 2x20000 FAIL=0 (seed1 PASS 6666->6789, seed2 6780->6724; net +67, the per-seed drift is novelty-trajectory from the now-active relocation, not a WRONG -- the curved post-condition gate is UNTOUCHED, confirmed by diff audit). DECLINE-never-WRONG intact.

SCOPE: PASSes under KEEL_WRAP_FLOW (which also gates the cyl/cyl wrap-bands that lack an independent oracle, so the default gate stays until cyl/cyl gets its certificate). The genuine POLE-encircling sphere wrap variant (not exercised by probe_wrap's contractible rings) DECLINEs; a dormant untested path for it was removed to keep the kernel clean. [[minimize-declines]] [[kernel-known-limitations]]

## Addendum 296: the two realsoak mass-WRONG classes SOLVED (fillet inner-loop stranding + disjoint mirror frame-handedness) -- both diagnoses CORRECTED their dossier; -296 fillet declines now PASS, WRONG=0 (2026-06-18)

The realistic-workflow soak (examples/realsoak.rs, the grammar-driven Plasticity fuzzer, 113-transcript model) surfaced 18 decline-never-wrong VIOLATIONS in its first 10k run -- NOT wrong bodies, but wrong analytic mass on CORRECT geometry the kernel returned without self-detecting (a floor fix first DECLINED them; this addendum makes them PASS). Round 0005's two mass dossiers (71, 72) are now SOLVED and integrated (branch fillet-mass-frontier), each by a focused fresh-context worktree agent that CORRECTED its dossier's premise with independent instrumentation, then re-verified and integrated here.

CLASS A -- post-fillet mass over-count (16 of 18; repro seed 11400715918834829014). Dossier 71 (and the prior in-session diagnosis) blamed the green-slab integrator mis-reading a trimmed cylinder. DISPROVED: per-face x-flux + Monte-Carlo generalized-winding witnesses proved massprops.rs was CORRECT all along (boundary x-flux = volume only for a CLOSED shell), and the body was NON-WATERTIGHT (open_ratio 0.021). The real defect is a B-rep SURGERY bug: split_face/mef_impl divides only the OUTER loop and strands inner-hole loops on face_old; the fillet's spring-line imprint / end-arc cap split then leaves the kept support on face_new and the later kef pulls a bore/window loop into the tiny blend cylinder (R=0.397 wrongly owning a R=3.68 bore). FIX (blend.rs; massprops.rs BYTE-UNCHANGED): redistribute_inner_loops(face_old, face_new) reassigns each hole loop to whichever split child geometrically contains it (even-odd in the support (u,v), periodic-u unwrapped), called from imprint_spring_line + split_blend_cap. Seed 829014: analytic 8405 -> 6082.9 == mesh 6080.0 (watertight), PASS. Broader than the 18: fillet floor-gate declines 1936 -> 1640 (-296 fillets now compute correct mass instead of declining).

CLASS B -- disjoint curved-union mass collapse (2 of 18; repro seed 11400715918834829910). Dossier 72 prescribed an import-time sense flip in combine_disjoint. DISPROVED for Keel: the sphere tessellator is sense-dependent, so flipping sense fixes mass but breaks the mesh the gate compares against. ROOT: a mirror reflects each surface frame, so the curved integrator's natural normal du x dv (a pseudovector) flips inward while the mesh's geometry normal (q - centre) does not -> mass and mesh want opposite sense -> the reflected lump's flux subtracts (V_a - V_b ~ 0). The planar path self-corrects via loop winding (which also reflects), so only the curved path collapsed. FIX (massprops.rs, at the source): fold frame handedness -- the curved-face dispatch passes sense_sign * frame_handedness(surf), a bit-identical no-op for proper frames (det +1) and the needed flip for reflected frames (det -1). Seed 829910: mirror 238 -> 15110.7 == mesh 14441 (curved band), PASS. Also closes a latent single-body bug: standalone mirrored curved bodies now compute correct analytic mass (was Err non-positive-volume).

CERTIFIED + GATE: both repros independently re-run after integration PASS; cargo test --workspace = 24 binaries / 0 failed (curved_volume_robustness 11 -- the green-slab baselines UNMOVED, since the green-slab is untouched; + 2 new regression files post_fillet_mass, disjoint_curved_mass); fmt + clippy clean; each agent's 3000-project soak WRONG=0, and the 10k final soak confirms WRONG=0 (counts in the session report). The DECLINE-never-WRONG floor gates (fillet return + disjoint-Union arm) stay INTACT as the safety net. Both fixes are independent files (71 blend.rs, 72 massprops.rs) and coexist.

CENSUS + DISPOSITIONS (the round's other findings). A full-message realsoak tally RETARGETED the fillet frontier: the fillet/chamfer declines are dominated by fillet:no-edge (a FUZZER artifact -- realsoak offers only straight 2-face edges, excluding curved cap-rims the kernel can fillet) and chamfer: cut failed (= a boolean(Difference) of the prism cutter failing -- the curved-boolean-assembler frontier, the SAME class as AssemblyFailed, chamfer.rs:103). Dossier 70's transverse multi-face end-cap cap does NOT occur in box CSG (degree-5 vertices are measure-zero; the real case is the parallel-cap extend), so its Section-1 agent code stays UNMERGED on its branch. The pre-existing fillet panic the dossier-70 agent reported did NOT reproduce on this base (7,422 oblique debug fillets, 0 panics; harness examples/probe_fillet_oblique.rs) -- it was base-specific. The deep backlog (chamfer-cut / AssemblyFailed / shell:Precondition / mirror:Precondition) is the curved-boolean / offset / NURBS-transform frontier: decline-safe (WRONG=0), the ongoing minimize-declines program, no same-day PASS fix. CI Actions were made frugal then fully DISABLED at the user's request (zero Actions until re-enabled). [[minimize-declines]] [[realsoak-wrong-frontier]] [[kernel-known-limitations]] [[fuzz-soak-completion-gate]]

## Addendum 297: the compound-operand boolean assembler -- mapped, two rungs landed (T-junction + seam-crosses-hole), the mass integrator PROVEN correct, the unified per-face overlay queued as research 0007 (2026-06-19)

The dominant remaining realistic-workflow decline is the compound-operand boolean (a simple tool unioned/cut into an already-multi-feature body): ~6k `extrudeU:AssemblyFailed` / `UnassemblableSeam` of the 66k kernel-frontier at 10k. A round of instrumented agents mapped it precisely and landed its tractable rungs (branch boolean-frontier -> master; WRONG=0 throughout).

LANDED (all WRONG=0, exact all-planar oracles, on master): fillet **surgery robustness** (the `redistribute_inner_loops` family extended to cap-rims via `rehome_dying_face_holes` + all-loops scans; the `mekr`/`no-cap-edge` decline classes eliminated; PASS 7851->7969); the **T-junction imprint** (`edge_on_face_containing`: split a tool wall's boundary edge where a second body-face seam lands mid-edge; -724 kernel-frontier declines at 10k); the **isolated seam-crosses-a-hole** (`try_imprint_chain_through_hole`: `mekr`-bridge the hole into the outer loop, then `split_face`).

PROVEN + LOCKED: the green-slab MASS INTEGRATOR is CORRECT -- the "watertight mass!=mesh" declines are the MESH over-reading a genuinely MALFORMED body (a FALSE `mesh_open_ratio==0` on radial-1 edges), adjudicated by an independent generalized-winding-number (GWN) Monte-Carlo volume that matches the analytic mass, NOT the mesh (kernel/73-FINDINGS sec 10; locked by tests/cyl_union_mass_witness.rs -- do NOT touch the green-slab). The dual mass-vs-tessellation gate is LOAD-BEARING for WRONG-prevention (relaxing it produced WRONG=1).

THE WALL + THE PLAN: the REAL compound cases stack a hole-crossing AND sibling-seam T-junctions on ONE face; the one-shape-at-a-time rungs do not compose (each freed imprint reaches the next seam stage). The unified fix is a **per-face planar overlay** of {outer loop, hole loops, all seam chains}, gated behind a **WRONG-safe watertightness witness** (fix the false-negative `mesh_open_ratio` + add the GWN/op-volume independent witness). Queued as research 0007 (#76 overlay, #77 witness), consolidating the 73/73b/73c FINDINGS. Chamfer (#74) + shell (#75) await their 0006 dossiers. Current soak: PASS 7969/10000, kernel-frontier ~65,978, WRONG=0; zero declines is the kernel-completeness endgame (multi-session), the overlay is the dominant next lever. [[minimize-declines]] [[realsoak-wrong-frontier]] [[kernel-known-limitations]]

## Addendum 298: the per-face planar overlay landed its all-planar beachhead and the co-wound-hole rung is CLOSED -- assembler declines -268, PASS 7984, WRONG=0 (2026-06-19)

Research 0007 #76 realized (boolean.rs, master 482d303). The per-face planar overlay (`try_imprint_chain_arrangement`) arranges {outer loop, hole loops, seam chains} on one face and re-knits the sub-faces via mev-spurs + `mekr`-bridge + `split_face`, subsuming the 73b T-junction and 73c hole-dip rungs. This round CLOSED the last open sub-case: the **co-wound dipped hole**, which the prior round DECLINED on the premise that reversing a live inner loop would break the shared hole edges' manifold pairing.

That premise was REFUTED by instrumentation (`KEEL_COWOUND_PROBE`): each shared hole edge is radial-2 (a hole fin + a wall fin), and the kernel's radial invariant (`check_radial_cycles`) constrains only that each fin sits in exactly one radial cycle -- it does NOT couple the two fins' relative `forward`; a wall face reads its OWN fin and `check_boundary_chains` is per-loop. So reversing the hole loop in place is a valid LOCAL op touching only that loop's fins. The fix: `reverse_inner_loop` flips a co-wound dipped hole to counter-wound, then the proven counter-wound re-knit runs unchanged (the `mekr` then merges it into the outer loop, so no co-wound inner loop survives). This is dossier 76 sec-4 option (a) -- inner-loop winding canonicalization -- realized at the cheapest correct site (the imprint's working operand where the dip is detected), NOT a global stitch pass. massprops.rs + blend.rs BYTE-UNCHANGED; every floor gate intact.

GATE CASE (exact, all-planar): seed `11400715918834826715` -- a face carrying a hole-crossing AND a sibling-seam T-junction -- now ASSEMBLES its compound Difference at **mass=12932.0964 == mesh=12932.0964** (oracle Difference 12932.10) and Intersection 3096.32; new regression `overlay_cowound_hole_dip_assembles_at_exact_oracle` forces the co-wound storage and asserts BOTH booleans exactly. SOAK (10k, seed 1, merged tree re-confirmed locally): PASS 7970->**7984** (+14), kernel-frontier 65945->**65677** (-268), the targeted unmatched-coedge class -477, **WRONG=0 [invariant HELD]** in both lanes (509 curved tessellation divergences, in-bound, NOT wrongs). NEXT LEVERS (distinct, by aggregate count): the general compound assembler (curved carriers / harder arrangements) is still dominant (~16k `*:AssemblyFailed`); then shell:Precondition (~5.9k), chamfer:Precondition+no-edge (~4.7k, awaits #74), mirror:Precondition (~4.4k), fillet:no-edge+Precondition (~4k). [[minimize-declines]] [[realsoak-wrong-frontier]] [[kernel-known-limitations]]

## Addendum 299: the PLANAR overlay frontier is EXHAUSTED -- the remaining planar imprint decline is a correct BLIND-CUT; the dominant boolean frontier is CURVED (mass!=mesh 61% + curved UnassemblableSeam 24%) (2026-06-19)

A census-grounded follow-on to the #76 overlay (logic-inert; master). Dispatched to build the multi-seam crossing rung; the agent REFUTED the premise and characterized the real frontier instead. The lead's target precondition `"crossing imprint: expected two seam fins at the crossing"` is NOT in the planar overlay -- it lives in `imprint.rs::imprint_closed_curve_crossing`, fires ONLY for a closed seam wrapping a cylinder/cone lateral, is ALWAYS co-located with `IntersectionFailed` (the curved-SSI frontier, out of scope), and its census bucket is **0**.

THE REAL BOOLEAN FRONTIER (failing attempts, 10k seed 1): degenerate `mass!=mesh` **62,912 (60.9%)** [curved tessellation + broken curved stitches -- forbidden to relax]; `UnassemblableSeam` **24,459 (23.7%)** [the curved partial-band/crossing-pair WRONG-prevention guard]; `unmatched-coedge` **15,776 (15.3%)**; the named planar precondition **0**. The largest PLANAR imprint decline (`imprint_open_chain`'s `"open chain end not on boundary"`) was probed (`KEEL_CHAIN_PROBE`): the declining chains have an endpoint DEEP in the face interior (1-12 units from any boundary, 204/219 over 1 unit) with NO sibling seam -- a **blind cut** (a tool partially penetrating a compound face). This is an IRREDUCIBLE, CORRECT decline: no boundary-to-boundary `split_face` exists, and forcing a spur is a dangling edge the mass==mesh gate rejects. The one tractable variant (endpoint on an inner hole loop) is ~0.15% and topologically delicate; left declined.

LANDED (logic-inert, soak BYTE-IDENTICAL: PASS 7984, frontier 65677, WRONG 0; commit 3936f23): the `KEEL_CHAIN_PROBE` reproduction rail + a permanent WRONG-guard test `blind_cut_interior_terminus_is_decline_never_wrong` (a box+boss battery asserting every partial-overlap result is a clean decline OR exactly-watertight, never wrong-positive) + docs 5c. massprops.rs + blend.rs untouched; lib 297 tests green.

REDIRECT (the worklist pivots off planar): the planar assembler overlay is COMPLETE -- its residual is correct blind-cut declines. The dominant remaining boolean frontier is CURVED. Next levers: (A) **the #77 GWN WRONG-safe witness** -- recover the tessellation-divergence slice of the 61% `mass!=mesh` class (curved bodies whose analytic mass is in-bound but the coarse mesh diverges; the soak's "509 curved divergences, NOT wrongs"), letting the gate ACCEPT a self-consistent result only when the independent generalized-winding-number volume confirms (PASS without admitting a wrong body); (B) the **curved-carrier SSI/stitch** frontier (the `UnassemblableSeam` guard + `IntersectionFailed`). [[minimize-declines]] [[realsoak-wrong-frontier]] [[kernel-known-limitations]]

## Addendum 300: FAITHFUL tutorial testing surfaced a real decline-never-WRONG violation the 2.24B-exec fuzzer missed -- curved boolean gate hole fixed (WRONG->DECLINE); tutorial scoreboard built, 7/12 (2026-06-19)

The pivotal correctness finding of the session, and a methodology correction. The `realsoak` FUZZER samples a tutorial-shaped grammar with RANDOM primitive dimensions, so ~70% of its ~65k "kernel-frontier" declines are geometrically-impossible-input ARTIFACTS, not real workflow failures. Built the FAITHFUL instrument the project had only planned: `tests/tutorial_workflows.rs` (the 6 dominant Plasticity workflow CLASSES + variants, run with SENSIBLE FIXED parameters, deterministic) plus a `KEEL_FAITHFUL` decline-PROVENANCE classifier on realsoak.

THE REAL WORKLIST (faithful provenance, 10k, fork-base 2bd0be9): **REAL declines 30,092 (29.8%)** vs **ARTIFACTS 70,729 (70.2%)** -- half the old "frontier" was noise (`input-not-watertight` 21,921, by-design `op-not-on-solid` 40,692, `no-target-edge` 4,222, `thickness-exceeds-wall` 3,043). Genuine worklist ~30k. Top real classes: boolean `*:AssemblyFailed` (dominant), fillet/chamfer/mirror `Precondition`, `*:UnassemblableSeam`.

THE WRONG (headline). A realistic `shell -> mirror -> fillet -> boolean(Union)` returned **Ok** with analytic **mass 10009.65 != mesh 5242.54** (a genus-1 body, 3 components that stayed disjoint) -- a **decline-never-WRONG violation on the CLEAN kernel** that the 2.24B-execution fuzz gate (Add 193) NEVER hit. Decline-never-wrong's DETECTION promise held (the oracle flagged it, never silently passed); the kernel's GATE had a hole. Root cause: the curved boolean gate (`boolean.rs` ~L5448) checked mass against `tessellated_volume()` -- a SINGLE-ref tessellation (~9995, which matched mass) -- but NEVER against per-component `mesh_volume()` (5242, the collapsed value). Independent Monte-Carlo truth = 10000 => the MASS was CORRECT, the BODY malformed. FIX (commit 0aa7676): the curved gate now ALSO requires `mass == mesh_volume()` (2% curved band, additive AND-condition) => the malformed union DECLINES (WRONG->DECLINE). Ironclad direct-API repro `tests/union_wrong_repro.rs` (public-API only). massprops.rs UNTOUCHED; nothing loosened (a strictly stronger gate). Verified: repro now declines, `three_bucket` N=2000 WRONG=0 both lanes, full suite + clippy green; consolidated 10k soak confirming WRONG=0.

TUTORIAL SCOREBOARD: `tests/tutorial_workflows.rs` is the live metric (the fuzzer cannot give it); each declining workflow is `#[ignore = "tutorial-gap: ..."]` (un-ignored as fixed). **6 -> 7/12**: curved-face hollow now shells a filleted box (`extrude_fillet_shell` passes; commit 77d30f8 = analytic curved-face offset + nested two-shell containment assembly). Adjacent 2-edge fillets made exact (held back -- it altered fillet output and broke hollow; A<->hollow interaction queued). Remaining gaps: adjacent fillet 3-edge corner + adjacent chamfer (coincident-face union), mirror+union (coincident plane), cyl+block fillet (curved pcurve bounds), G2 fillet (a missing feature).

ROBUSTNESS finding: deep realistic chains drive the recursive boolean/tessellation deep enough to OVERFLOW the thread stack -> a non-catch_unwind ABORT (overflowed even a 256 MB stack on one project). The recursion must be made iterative or depth-bounded so it DECLINES gracefully instead of crashing -- a real risk for deep histories (relevant to the fieldforge swap). Queued.

SIGNIFICANCE: the "correctness program COMPLETE / WRONG=0" milestone (Add 193) was true for the FUZZER's distribution; realistic workflows still surface gate holes the random distribution misses. Faithful tutorial testing is now the primary correctness instrument. One faithful run found in minutes what billions of random executions did not. [[faithful-tests-over-proxy]] [[realsoak-wrong-frontier]] [[geometry-kernel-project]] [[minimize-declines]]

## Addendum 301: the deep tutorial-gap grind -- 6 -> 9/12 faithful workflows; two more landed (chamfer-all via a real f64 clipper bug, mirror+union), two hardened-but-characterized; WRONG=0 throughout (2026-06-19)

Continued the faithful tutorial worklist (Add 300) by grinding the declining workflows one agent at a time, each WRONG-safe and soak-gated. Net **tutorial scoreboard 6 -> 9/12** (master 7847a47), every step holding WRONG=0 on the consolidated 10k soak (PASS ~7990-8028).

LANDED (un-ignored, permanent regression guards in `tests/tutorial_workflows.rs`):
- **extrude->fillet->shell** (curved-face hollow; commit 77d30f8): `offset_body` generalized to curved faces (`offset_analytic_surface` + LM 3-surface vertex solve in tweak.rs) + a nested two-shell `combine_containment` assembly (boolean.rs) that bypasses the SSI gap; shells a filleted box, mass==mesh. 6->7/12.
- **mirror_solid_union** (coincident-plane union; commit 1093ba7): root cause was `Body::mirrored` leaving reflected surface frames LEFT-handed -- `face_outward_normal_at` read `su x dv` (a pseudovector, inverts) while mass/tessellate read `frame.z` (correct), so the classifier mis-read the flush mirror faces. Fix: fold `surface_frame_handedness` into the analytic normal (dossier-72 correction; +1.0 no-op for every right-handed body). 7->8/12. This SAME bug was the root of the earlier mirror-union WRONG, now a correct PASS.
- **chamfer_even_top_edges** (chamfer all 4 top edges; commit 7847a47): a real f64 CLIPPER bug, not a stitch bug. `clip_line_to_planar_face` (boolean.rs) used a FIXED parallel-guard `denom.abs() < 1e-300`; when an SSI seam line runs exactly ALONG a cutter face's boundary edge (denom rounds to ~2e-14, a 0/0), a garbage finite `t` truncated the chord -> orphaned seam coedges -> "cut failed". Fix: a GEOMETRY-RELATIVE parallel tolerance `par_eps=(|n|*|d|).max(1)*1e-9`. Latent clipper-wide hardening (any transversal cut whose SSI chord lies on a cutter edge), not chamfer-specific. 8->9/12.

HARDENED but still CHARACTERIZED (deep surgery/feature; left declining, never wrong):
- **fillet ALL 12 box edges** (commit 842ae1d): replaced the spring-REUSE (which collapsed a 2-edge corner to a 4-valent vertex `hollow` can't offset -- the trap that broke an earlier attempt) with an ENDPOINT-INCLUSIVE crossing that builds the correct 3-valent bicylinder corner, so adjacent 2-edge fillets are robust AND shell-safe (box reaches 4/12). The 3-edge sphere-octant corner is geometrically PROVEN (mass-exact for 4/8 corners) but the cube's interleaved fillet order would risk a 0.76%-off body at edge 8, so a `both_cyl` WRONG-guard DECLINES it (removing ~36 band-tolerated near-wrong corners from the soak: PASS 8028->7992). Full octant trim/stitch for interleaved corners is the coupled follow-up.
- **cyl+block fillet** (boss cap-rim; commit 08940af): the concave boss/plate seam fillet needs the OUTWARD torus (major=r_cyl+radius) + concave-aware cap-rim surgery (imprint_ring/mekr/ring-selection are convex-specific). The convex code mis-built it; replaced a confusing late `mass_properties` error on a broken body with an HONEST early `fillet_edge` decline (GWN boss-detection probe). 3-step fix plan documented in findings.

REMAINING tutorial gaps (3/12): fillet-all 3-edge octant (interleaved trim), concave boss cap-rim (concave surgery), and **G2 fillet -- a missing FEATURE** (the API is radius-only/G1; curvature-continuous fillets need a new surface family, not a decline-fix -> documented as a known feature-gap, out of scope for the decline program). Plus the ROBUSTNESS item from Add 300: deep chains STACK-OVERFLOW->abort (depth-bound the recursion).

These 9/12 are a strong AUTOMATED baseline; the residual is exactly what roadmap #4/#5 (manually run ~100 tutorials, log+patch) will prioritize against real usage. [[faithful-tests-over-proxy]] [[realsoak-wrong-frontier]] [[minimize-declines]] [[geometry-kernel-project]]

## Addendum 302: the concave boss cap-rim fillet LANDED -- tutorial 9 -> 10/12; roadmap #1 at its automated ceiling (2 residual = a deep follow-up + a missing feature) (2026-06-20)

The hardest remaining surgery gap is solved (commit 04d82d4, master). `boolean_then_fillet_cyl_block_union` (union a cylinder boss onto a plate, then fillet the CONCAVE circular boss/plate seam) now PASSES -- mass 45201.5 == mesh 45170.7 (in [44500,45700]). Four-part fix, all WRONG-safe, the convex cap-end fillet path + hollow untouched:
1. **`blend_torus_for_edge`** builds the OUTWARD torus for the concave branch (major = r_cyl + radius = 15, tube above the plate / outside the cylinder; the old R>2r guard was convex-only); a GWN material-side probe sets a `concave` flag.
2. **`edge_is_convex`** rewritten as a direct GWN DIHEDRAL measurement (fraction of a perpendicular-plane circle inside the solid) -- the old centroid-of-outer-loop heuristic sat over the annular plate's bore and mis-read the boss seam as convex. No winding/centroid/normal assumptions now.
3. **`tweak.rs::merge_two_arcs_to_closed`** (new Euler op) -- the dominant blocker the prior characterization MISSED: a boolean Union splits the bore circle into TWO cocircular arcs (cylinder-seam endpoint + a valence-2 antipode), unlike the convex rim's single closed circle; this dissolves the antipode and fuses them so the cap-rim recipe transfers.
4. **`fillet_cap_rim_concave`** (convex surgery with loop roles inverted: rim is the INNER hole, torus ring sense reversed) + **`tessellate.rs::torus_tube_span`** fixed to pick the v-quarter by the periodic largest-gap span (the naive min/max took the complementary 270 deg across atan2's pi branch cut, so mesh != mass). Ends with a `validate() + mass==mesh` self-check that DECLINEs on any imperfection (>2-arc rims, cyclides, over-large radii) -- never a wrong body.

Verified: tutorial 10/12, full suite green (298 lib + 30 blend + 3 convex cyl-rim-to-torus + all WRONG-locks), clippy clean, consolidated 10k soak WRONG=0.

ROADMAP #1 AT ITS AUTOMATED CEILING (10/12). The 2 residuals are NOT decline-fixes: (a) **fillet-all 3-edge octant** -- the sphere-octant is geometrically proven (exact 4/8) but the cube's interleaved fillet order needs a generic-cap trim follow-up (a deep coupled surgery); (b) **G2 fillet** -- a missing FEATURE (no curvature-continuous fillet API; needs a new conic/NURBS blend surface family). Both are documented out-of-scope for the decline program and are natural roadmap #4/#5 (manual tutorial pass) territory. Session arc: tutorial 6 -> 10/12, a real decline-never-WRONG fixed, an f64 clipper bug fixed, blend + mirror + hollow + boss-fillet all hardened, WRONG=0 throughout. NEXT: loose ends (#77 GWN-witness reconcile+merge; the deep-chain stack-overflow depth-bound) then roadmap #2 (technical docs). [[faithful-tests-over-proxy]] [[realsoak-wrong-frontier]] [[minimize-declines]] [[geometry-kernel-project]]

## Addendum 303: crash-safety -- the two unbounded recursions on the boolean path made ITERATIVE (byte-identical); deep-chain stack-overflow hole closed; #77 deferred (2026-06-20)

The loose-end from Add 300/302: realistic deep op-chains could overflow the thread stack -> a non-catch_unwind ABORT. A crash is strictly WORSE than a decline -- it bypasses the entire DECLINE-never-WRONG contract (the harness/caller cannot catch it). An agent surveyed every recursive path on boolean/tessellation/validate/mass and found EXACTLY TWO genuinely unbounded recursions, both now iterative (commit on master; byte-identical output for all valid inputs):
- **`connected_face_components::find`** (boolean.rs, the boolean ASSEMBLY hot path, called from `finalize_imported_sheet`/`finalize_imported_assembly`): a recursive path-compressing union-find whose depth = union-tree height; because the union step was arbitrary (not by-rank), a long union chain degenerates the tree to O(component-size) depth -> unbounded in body complexity. Rewritten to the iterative two-pass find-root-then-compress already used by `seam_components::find` (same root, same full compression).
- **`Assembly::walk`** (assembly.rs): recursive DFS over the assembly DAG; depth = nesting depth. Rewritten to an explicit-stack pre-order DFS (`walk_iter`, children pushed reversed) carrying each frame's instance-id path + composed world transform (occurrence identity/placement unchanged).
Everything else was already iterative or bounded-shallow (earclip n^2 guards; SSI marchers `for _ in 0..N`; Bernstein `roots_rec` t_tol floor depth ~30; other union-finds already iterative).

The exact 256 MB overflow could NOT be reproduced via a direct-API path (a clean 806-face/genus-100 body runs every per-op routine on a 256 KiB stack; ~1,600 deep chains survived a 1 MB stack) -- so it is a rare degenerate-configuration runaway, and the iterative conversions eliminate the only unbounded recursions found on/near the boolean path (the right defensive posture even without the exact repro). Regression `tests/deep_chain_no_abort.rs` runs boolean+validate+mass+mesh on a heavy body on a 256 KiB worker thread and asserts the thread joins (an overflow would abort the process). `KEEL_WORKER_STACK_MB` + `KEEL_CRASH_HUNT` added to realsoak so the orchestrator can dial the worker stack down and pin any future crashing seed. Verified: full suite 351 passed / 0 failed, all WRONG-locks green, tutorial 10/10, clippy clean, consolidated 10k soak PASS 8039 WRONG=0 (byte-identical to pre-fix); massprops untouched.

#77 (the GWN watertightness witness) DEFERRED: it lives unmerged on `boolean-frontier` (86e4e0d) but forked early and its boolean.rs gate edits overlap the `mass==mesh_volume` gate already on master (commit 0aa7676), which subsumes much of #77's malformed-body-detection purpose. Reconciling its stale gate is not worth the risk for latent infra; revisit only if an explicit watertightness net is later needed. With this, BOTH loose ends are resolved (stack-overflow fixed; #77 deferred-with-rationale), and roadmap #1 (tutorial declines -> zero) is complete at its automated ceiling 10/12. [[realsoak-wrong-frontier]] [[geometry-kernel-project]] [[minimize-declines]]

## Addendum 304: roadmap #2 -- technical documentation phase complete (capabilities + architecture + public-API rustdoc); doc-only, no behavior change (2026-06-20)

The documentation phase (the user's roadmap #2) landed three complementary docs on master, all doc-only with zero behavior change (suite green throughout):
- **`docs/CAPABILITIES.md`** -- the honest capabilities + LIMITATIONS frontier: the contract restated, the WRONG=0 evidence chain (1M oracle + 2.4B fuzz + the faithful realsoak), the live faithful **tutorial scoreboard (10/12)** with the per-workflow table, the known-limitations frontier (3-edge fillet corner, G2 feature-gap, the compound-operand assembler, curved SSI, NURBS booleans, scope-by-design), the decline-provenance result (~30% of fuzzer declines are real, the rest noise), the robustness posture, and how to verify locally. Every number cites a LOG addendum; written in the under-claim style. Complements (does not duplicate) README's S/P/D capability matrix.
- **`docs/ARCHITECTURE.md`** (660 lines) -- a source-grounded architecture overview deeper than README's crate summary: the exact-combinatorics-over-tolerant-metric inversion, the keel-math predicate/interval/Bernstein/OneRoot foundation, keel-geom curves/surfaces, the keel-topo radial-edge non-manifold B-rep data model (Fin/Edge.radial/Region/the key scheme), the Euler operator set (incl. the genus-handle pair), the boolean pipeline (with an ASCII flow diagram), blends/fillets, the Green-slab mass integrator, edge-first tessellation, and the correctness architecture (three-bucket oracle + dual mass-vs-mesh gate + GWN witness). Reconciliation notes folded in: surfaces-of-revolution are NURBS-realized (not a 6th analytic variant); the full Euler set includes mfkrh/kfmrh.
- **public-API rustdoc** (20 source files, +407/-46, ZERO logic change): doc-comments across the consumer surface (`Body` + primitive builders + modeling ops, the `boolean`/`BoolOp`/`BoolResult`/fault types with the DECLINE-never-WRONG contract spelled out on the entry points, `mass_properties`/`MassProps` with units, `validate`/`ValidationError`, the entity tower, and the keel-geom/keel-math signature types). Bonus: fixed 27 pre-existing broken intra-doc-link warnings, so `cargo doc --workspace --no-deps` now builds with **0 warnings (down from 27)**. Verified: workspace build + cargo doc clean, full suite incl. doctests green, clippy no new warnings.

Roadmaps #1 (tutorial declines -> automated ceiling 10/12, WRONG=0) and #2 (technical docs) are complete. NEXT: roadmap #3 (build fieldforge on the new kernel) -- the capabilities + API docs are its on-ramp. [[geometry-kernel-project]] [[fieldforge-sibling]]

## Addendum 305: GitHub landing-page refresh -- "At a Glance" correctness charts on the README (benchmarks deliberately omitted) (2026-06-20)

The README landing page now leads with a visual "At a Glance" section. A new generator `docs/media/render_charts.py` (matplotlib -> PNG, dependency-clean, every value cited to a LOG addendum inline) emits five figures into docs/media/: a verification-scale strip (2.4B+ fuzz / 1M oracle / 10K realsoak / 0 wrong), the randomized-oracle WRONG=0 bars (Add 193), the faithful tutorial scoreboard 10/12 (Add 302-303), the realistic-workflow soak donut 8039/10000 WRONG=0 (Add 303), and the decline-provenance ~30%-real split (Add 300). README also cross-links the new `docs/CAPABILITIES.md` and `docs/ARCHITECTURE.md`. Under the under-claim doctrine: every figure traces to an addendum, ratios round down, and PERFORMANCE BENCHMARKS ARE DELIBERATELY OMITTED with an explicit note ("no timing numbers are claimed on this page") until the keel-topo operation-bench harness exists -- the user said skip the benchmark, not defer the page. Charts regenerate with `python docs/media/render_charts.py`. The landing page can be re-enriched (more charts, op-benchmarks, a Pages site) once the bench harness + the fieldforge integration land. [[geometry-kernel-project]] [[landing-page-roadmap]] [[underclaim-publicly]]

## Addendum 306: Methodology & References doc (docs/REFERENCES.md), verified against source -- and two README MISCITATIONS corrected (2026-06-20)

Added a subsystem-mapped bibliography `docs/REFERENCES.md` + a short README "Methodology and References" section linking to it. The discipline was the point: every citation was CONFIRMED in the code at a named location, not pulled from memory, under the under-claim doctrine ("follows X", Keel claims the engineering not the invention). Verified-and-cited methods: Shewchuk adaptive predicates via the `robust` crate (predicates.rs); the Devillers-Fronville-Mourrain-Teillaud exact one-root comparison + CGAL Ak_1 interface + EGC contract (algebraic.rs); one-ulp outward interval arithmetic (interval.rs); Blinn-stable quadratic + Yuksel HPG-2022 monotonic-interval cubic (poly.rs); Farouki-Rajan 1988 Bernstein product (bernstein.rs); Sherbrooke-Patrikalakis 1993 Projected-Polyhedron system solver (multibernstein.rs); de Boor A3.1 NURBS eval / The NURBS Book (nurbs_curve.rs); Weiler radial-edge non-manifold B-rep + Requicha/Laidlaw-Trumbore-Hughes regularized set ops (boolean.rs); generalized winding number with Van Oosterom-Strackee per-triangle solid angle, Jacobson et al. 2013 classification (winding.rs); divergence-theorem green-slab mass integrator (massprops.rs, textbook).

The verification pass CAUGHT TWO public miscitations in the README, both corrected: (1) the one-root algebraic layer was described as "an expression DAG with BFMSS separation bounds" -- it is NOT an expression DAG (explicitly not built) and uses DFMT, not BFMSS; (2) the winding number was attributed to "Spainhour-Weiss" but winding.rs implements the triangle-based GWN (Van Oosterom-Strackee + Jacobson et al.). ARCHITECTURE.md was already correct on both (it was source-grounded). Lesson reinforced: verify public claims against the code; a miscite is worse than an omission. Doc-only; no kernel change. [[underclaim-publicly]] [[geometry-kernel-project]]

## Addendum 307: first labeled release -- Alpha 0.1 (tag v0.1.0-alpha) (2026-06-20)

The public label moved from "pre-alpha" to **Alpha 0.1**. README tagline updated; annotated git tag `v0.1.0-alpha` pushed (workspace version already 0.1.0); GitHub pre-release "Alpha 0.1" published at releases/tag/v0.1.0-alpha with honest, LOG-traceable notes (WRONG=0 / 1M oracle / 2.4B fuzz / realsoak 8039-of-10000 / tutorial 10-of-12; shipped ops; links to CAPABILITIES/REFERENCES/ARCHITECTURE; GPL-3.0-or-later + commercial). No code change. [[geometry-kernel-project]] [[underclaim-publicly]]

## Addendum 308: public edge/face enumeration + pick API (the fieldforge feedback loop's first item) (2026-06-20)

The fieldforge integration session hit a real blocker: you could fillet an edge of a freshly-built PRIMITIVE (its constructor returns edge keys) but could NOT enumerate or pick an edge of a BOOLEAN result, because edge/face enumeration + pick was not public (entity_ids+lookup were public but clunky across the WASM boundary; face_keys/face_loop_edges were pub(crate); no edge-level pick). This is the prompt's designed feedback loop working: fieldforge surfaces a need, Keel implements it under its gates. Additive, read-only, zero geometry-behavior change.

Added (all `pub` on `Body`): `edge_keys(&self) -> Vec<EdgeKey>` (body.rs); `face_keys(&self) -> Vec<FaceKey>` (boolean.rs, widened pub(crate)->pub); `closest_point_on_edge(&self, e: EdgeKey, p: Vec3) -> Option<(Vec3, f64)>` and `nearest_edge(&self, p: Vec3) -> Option<EdgeKey>` (interrogate.rs, alongside closest_point). EdgeKey/FaceKey were already public. closest_point_on_edge is curved-aware: exact segment projection for straight edges; clamped closed-form Circle3/Ellipse3 projection for conic edges (reusing the arc-span rules); keel-geom's certified global project_point (Bezier + AABB B&B + bracketed Newton) for NURBS; endpoints + bounded polyline samples as fallbacks. nearest_edge = min-distance edge, lowest-EntityId tie-break. All rustdoc'd.

Test `tests/edge_pick_api.rs` (3, green) proves the exact blocked flow: union two blocks -> edge_keys() -> nearest_edge(seam point) -> fillet_edge -> watertight, mass==mesh; plus straight + curved (cylinder cap circle) closest-point exactness. Full suite + clippy + cargo doc green; WRONG-locks intact; no soak needed (read-only accessors). Noted: filleting the stepped-corner VERTEX edge legitimately DECLINES (kev: fins not adjacent, a surgery limit, never WRONG) -- the pick works on every edge regardless. DEFERRED to the keel-wasm/Phase-2/3 work: worker_mesh edge_groups (per-edge polylines for UI highlight) + worker_mesh_tol(chord) (draft vs final tessellation); closest_point_on_edge is the building block for ray-pick. [[geometry-kernel-project]] [[fieldforge-sibling]]

## Addendum 309: worker_mesh edge_groups + worker_mesh_tol (KEEL-TOPO capability); keel-wasm exposure handed to the fieldforge session (2026-06-20)

Feedback-loop item #2 (queued in Add 308). The KERNEL-side capability landed additively on master; the default worker_mesh()/mesh_volume/mass path is byte-identical (arc_segments(None,32)==32; snapshot test asserts worker_mesh().lines == render_mesh().edges), so all WRONG-locks stay green and no soak was needed. Added (keel-topo): `WorkerMesh.edge_groups: Vec<(u64,u32,u32)>` = (id, start, count) where id = the source edge's EntityId.0 (host maps via lookup(EntityId(id)) -> AnyKey::Edge), start/count in f32 ELEMENTS of the existing `lines` buffer (count always a multiple of 6; ranges contiguous, non-overlapping, tile the whole buffer, order unchanged); `Body::worker_mesh_tol(&self, chord: f64) -> WorkerMesh` (chord-tolerance variant for draft-vs-final) with worker_mesh() the unchanged default; both delegate to a private worker_mesh_opt(Option<f64>); edge tessellation via a new Body::edge_polyline_opt(edge, Option<f64>) (interrogate.rs) routing conic counts through arc_segments (widened pub(crate)). New test tests/worker_mesh_edge_groups.rs (3, green). keel-topo 298 lib + new tests green; massprops untouched.

COORDINATION NOTE: the fieldforge session has UNCOMMITTED keel-wasm work in the main repo (kw_fillet_nearest/kw_chamfer_nearest using the Add-308 pick API, + an untracked phase1_smoke.mjs). To avoid clobbering it, only the keel-topo capability was landed here; the keel-wasm EXPOSURE (per the worker_mesh agent's commit 4904481: exports `mesh_edge_groups_ptr`/`mesh_edge_groups_len` flattened (id,start,count) u32 triples, and `demo_mesh_build_tol(chord: f64)`, via an EDGE_GROUPS thread-local) is handed to the fieldforge session to integrate into its working keel-wasm/src/lib.rs (it owns that binding). The agent's full wasm32 build + run_spike.mjs validated edge_groups partition the lines buffer and the chord tol refines (coarse 336 tris / fine 7216). [[fieldforge-keel-worklist]] [[fieldforge-sibling]] [[geometry-kernel-project]]
