...
> **SUPERSEDED NOTE (manifold-first verdict reversed).** This wave-1 dossier recommends a manifold-first topology core (see the ADOPT item on arena/handle coedge storage and the INVESTIGATE verdict that manifold-first is correct). The kernel-layer research that followed (docs/research/kernel/01-topology-euler-operators.md and 02-nonmanifold-cellular.md) reversed this: spec decision D3, recorded in docs/research/01-synthesis-wave2.md (verdict V1) and docs/research/00-synthesis-v2.md (delta 11), adopts a non-manifold-capable representation, operator vocabulary, and validator from the first commit (PES-class, regions first-class), with manifold-semantics-first only as implementation ORDER. Where this document says "manifold-first is the verdict," read it as superseded by D3. The rest of this dossier (tolerant modeling, fuzzy booleans, construction-time invariants, the regression corpus, the geometry/topology split) stands.
