...
**6 of 12 faithful tutorial workflows PASS, 6 DECLINE.**
...
| 12 | `extrude_fillet_shell` | 6 extrude_fillet_shell | DECLINE | `hollow` Err `Precondition("offset_body: non-planar face")` | `hollow` cannot offset the CURVED fillet faces produced upstream |
...
## What the 6 declines implicate (the prioritized worklist)
...
2. **Curved-face followthrough after a boolean/fillet** -- a curved (cylinder)
   ...
   `mass_properties` declines (#2); and `hollow` cannot offset a non-planar
   (fillet) face (#6). Two ops (`mass_properties`, `hollow`/`offset_body`) both
   trip on curved faces created by an upstream feature.
