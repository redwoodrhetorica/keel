...
**The differentiation thesis:** exact or certified topology decisions over tolerant metric geometry, routed by an affordability gradient (exact for linear and conic geometry, exact-when-cheap for quadrics, certified-numeric for splines). This hybrid is the consensus recommendation of three decades of robustness literature, is delivered by no shipping kernel, commercial or open, and rests entirely on published patent-clean foundations. Tolerances never decide combinatorial topology.
...
- **Curved-predicate tiers (per docs/research/kernel/11, the doctrine-completing dossier).** Tolerances are NEVER used to decide combinatorial topology; they carry metric geometry and serialization only. Decisions route to the cheapest sound tier by algebraic degree:
  - *Tier 0, linear:* Shewchuk/indirect exact predicates (in place).
  - *Tier 1, circles and conics (degree <= 4):* genuinely exact and affordable. One-root algebraic numbers, resultant-derived sign sequences (Devillers-Fronville-Mourrain-Teillaud for circles; EXACUS/CONIX for conics), wrapped in a lazy-exact number type (interval first, exact on demand, memoized) behind semi-static filters, with Wein-style high-level filtering (carry defining equations, refine on demand) for arrangements. Powers the M4 2D profile subsystem exactly.
  - *Tier 2, quadrics:* QI pencil analysis (Dupont-Lazard-Lazard-Petitjean) for intersection curves; exact predicates when QI yields rational or single-sqrt parameterizations (always true for singular intersections), certified intervals otherwise; full exact 3D quadric arrangements reserved for offline verification, never the interactive path.
  - *Tier 3, NURBS and high degree:* exactness is unaffordable (separation bounds explode, inputs are floats anyway). Certified-numeric topology instead: Bernstein subdivision isolation, Plantinga-Vegter-style certified subdivision for region/curve topology, validated interval-Newton/Krawczyk continuation for SSI, with rigorously propagated error bands on the tolerant metric geometry. Exact-topology-plus-tolerant-geometry is the hybrid the field endorses and no shipped kernel delivers: Keel's opening.
  - *Cross-cutting:* one Rust `AlgebraicReal` expression-DAG number type (interval approximation + BFMSS/Li-Yap separation-bound escalation, LEDA-real/CORE-Expr architecture) serves Tiers 1-2; the curved algebraic layer is specified to the CGAL Algebraic Kernel interface (Ak_1: isolate/compare/refine/sign_at; Ak_2: Bisolve solve_bivariate, curve topology); filter constants are property-tested. Cost reality to honor: CAD inputs are degeneracy-dense ON PURPOSE (tangent fillets, coincident faces, symmetry), so filter-failure paths are hot paths, not tails.
...
- The surface enum leaves headroom for a `FacetSurface` variant (Parasolid Convergent Modeling lesson): mesh faces as first-class topology citizens later, without rearchitecting dispatch. **Patent constraint (D10):** single-body mixed mesh/B-rep operations are covered by an active Siemens family (~2035+); the shipping path until then is mesh and B-rep as separate bodies converting at boundaries via published mesh-arrangement booleans. The enum headroom costs nothing and positions us for expiry.
...
- **Implementation doctrine:** the full determinism contract and file-format doctrine in `docs/research/kernel/14-determinism-serialization.md` (as summarized in `docs/research/02-synthesis-wave3.md`) are binding: pure-Rust `libm` for determinism-critical transcendentals, reproducible parallel reductions, no hash-order/pointer leakage into output, exact f64 round-trip serialization (raw bits canonical, ryu for text), append-only versioned schema with down-save, STEP AP242 as the must-implement interchange, importers fuzzed as adversarial input, durable definition + rebuildable cache. Cross-compiler and cross-platform differential CI over the regression corpus enforces this contract from M1.
- **Additional kernel obligations from the regeneration/assembly/PMI boundary studies (wave 3):** cheap snapshot/restore of body state; structured non-fatal operator failure (failed op leaves the body unchanged); first-class datums/construction geometry and programmatic geometric selectors; concurrency-safe operators over disjoint bodies; instancing via structural sharing with a first-class composable location primitive; local-coordinate geometry with double-precision placement (no world offsets baked into coordinates); a typed, versioned attribute system capable of carrying semantic PMI bound to persistent references.

### D10. Patent posture

*(Added per wave-3 research, docs/research/kernel/18. Informational survey, not legal advice; FTO review with counsel before shipping any flagged feature.)*

- The classical core (NURBS machinery, Euler/B-rep, classic booleans, classic blends including setback and variable-radius, tessellation, healing, subdivision) is expired or never-patented prior art: implement the published versions and cite them.
- **Three HIGH-risk zones with adopted safe alternatives:** (1) auto-inferred constraints during direct edits (Siemens Live Rules family, ~2034): ship explicit user-applied constraints instead; (2) mixed mesh+B-rep in one body (Siemens Convergent family, ~2035+): separate bodies, boundary conversion; (3) U-splines (Coreform, ~2037): use THB-splines, or classic T-splines whose original patent expired March 2024.
- Implicit/field-driven latticing (nTopology et al.) and cloud co-editing (Onshape/PTC) are live areas touching only optional layers above the kernel; the underlying published math is free.
- License: dual MIT OR Apache-2.0 (already adopted) for the Apache patent grant and retaliation clause.
- **Standing rule: defensive publication.** Novel Keel algorithms get dated public technical notes before any competitor could file.
...
Six literature-review tracks with full citations live in `docs/research/` (robust numerics, NURBS/freeform, surface intersection, booleans/tolerant modeling, kernel architecture lessons, ML for geometry), consolidated in `docs/research/00-synthesis.md`. **Wave 2** added two deep-dive collections: `docs/research/kernel/` (10 dossiers: topology/Euler, non-manifold/cellular, local ops, constraints/2D profiles, tessellation, interrogation/HLR, persistent naming/sessions, kernel engineering, mesh/implicit hybrid, Minkowski/medial/midsurface) and `docs/research/nurbs/` (6 dossiers: foundations, core algorithms, fitting, intersection/projection, offsets/blends/sweeps, applications/extensions), consolidated in `docs/research/01-synthesis-wave2.md`, which records two verdict changes applied to this spec (D3 non-manifold/regions-first; D9 lineage/determinism/sessions) and the shared-engine consolidation map. **Wave 3** (kernel/11-19: curved exact predicates, direct modeling, healing, determinism/serialization, feature recognition, assembly, PMI, patents, regeneration) is consolidated in `docs/research/02-synthesis-wave3.md`: it completed the D2 predicate tiers, added D10 (patent posture), made the differentiation thesis explicit in the mission, and bound the determinism/file-format doctrine plus the kernel-boundary obligations into D9. FP-contraction policy (wave-2, A8): no fast-math flags ever; `mul_add` only where intended; predicate code isolated from FMA contraction. The synthesis lists the binding design deltas applied to this spec and a flagged reading list per milestone (Golovanov before M3, Patrikalakis-Maekawa-Cho before M4/M5, Yang-Jia-Yan before M5, the mesh-arrangement corpus before M6). Topology future-proofing note from track E: the radial cycle around each edge is structured to hold 3+ coedges so partial-entity non-manifold support remains an extension, not a rewrite; persistent/immutable arenas are flagged as a candidate for Parasolid-style session rollback. **Round 3** (kernel/20-24: sheet metal, kinematics/motion, model diff/comparison, metrology-grade fitting, canonical geometry recovery, ~110 sources) closed the remaining literature gaps; it and a consolidated cross-round synthesis live in `docs/research/00-synthesis-v2.md` (deltas 9-22), whose unique contributions to this spec are the keel-tess/keel-io crate contracts, the comparison-cascade and pass-rate testing oracles, the expanded post-M7 long tail with confirmed tier boundaries, and the patent/determinism risk entries. Where synthesis v2 overlaps the wave syntheses, the wave-derived spec text governs.
